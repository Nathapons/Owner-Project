...
class SmtIqiProgram:
    ...
    def main_window(self):
        ...
        creator_label = Label(big_frame, text='Create by Nathapon.S IoT Dept.', font=self.creator_font)
    ...
    def pull_force_link_tree(self, event):
        ...
        status_tree = self.pull_force_treeview.item(input_id)["values"][3]
        report_status, pull_force_format_full_path = self.search_format_file(item_code, fetl_lot)

        if status_tree == "อัพเดทไปแล้ว":
            webbrowser.open(pull_force_format_full_path)
        else:
            msb.showwarning(title='แจ้งเตือนไปยังผู้ใช้', message=f' FETL no {fetl_lot} เปิดไม่ได้เนื่องจากไฟล์เสียหรือมีคนใช้งานอยู่')
        ...
        for file_name in os.listdir(pull_force_path):
            if str(file_name).endswith('csv'):
                ...
                if report_status == "Update" and "~$" not in str(file_name):
                    ...
                    if format_book != "" and format_sheet != "":
                        ...
                        report_status = "ไฟล์เสียหรือมีคนใช้งานอยู่"
    ...
    def copanarity_link_tree(self, event):
        ...
        status_tree = self.copanarity_treeview.item(input_id)["values"][3]
                    ...
                    if item_code == item_code_folder:
                        ...
                        for fetl_lot_file in os.listdir(link2):
                            if fetl_lot in fetl_lot_file:
                                ...
                                print(fetl_lot, fetl_lot in os.listdir(link2), os.listdir(link2))
                                report_status = 'Update'
